...
function CharactersSection({ characters }: CharacterProp){
    ...
    return(
        <>
            <div className="mx-4 lg:mx-6 flex flex-col">
                ...
                <div className="w-full max-w-6xl grid grid-cols-2 gap-4">
                    {characters.characters.map((character, index) => {
                        return <div key={index} className="flex flex-col gap-2">
                                    ...
                                    <div className="text-lg md:text-xl font-bold text-zinc-600 break-word">{character || "The Legend of Zelda"}</div>
                                </div>
                    })}
                </div>

                <div className="border-b border-gray-300 w-full mt-8"></div>
            </div>
